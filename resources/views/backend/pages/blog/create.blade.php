...
<div class="container-fluid px-4">
    <main>
        <div class="container-fluid px-4">
            ...
            @if (isset($blog))
            <form action="{{ route('admin.blogs.update', ['blog' => $blog->slug]) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                @else
                <form action="{{ route('admin.blogs.store') }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group row mt-5">
                        ...
                        <div class="col-sm-10">
                            <input type="text" id="title" name="title" class="form-control" placeholder="Please enter title" @if (isset($blog)) value="{{ old('title', $blog->title) }}">
                            @else
                    ...
                    <div class="form-group row mt-5">
                        ...
                        <div class="col-sm-10 ">
                            <select class="form-select changeUserStatus" aria-label="Default select example" name="category_id">
                                <option value="default"> Please select a Category </option>
                                @foreach ($categories as $category)
                                <option value="{{ $category->id }}" @if (isset($blog)) {{ $category->id == $blog->category->id ? 'selected' : '' }} @endif>
                    ...
                    <div class="form-group row mt-5">
                        ...
                        <div class="col-sm-10">
                            <input type="text" id="author" name="author" class="form-control" placeholder="Enter author name" @if (isset($blog)) value="{{ old('author', $blog->author) }}">
                            @else
                    ...
                    <div class="form-group row mt-5">
                        ...
                        <div class="col-sm-10">
                            <select class="form-select changeUserStatus" aria-label="Default select example" name="status">
                                <option value="default"> Please select a status </option>
                                <option value="publish" @if (isset($blog)) {{ $blog->status == 'publish' ? 'selected' : '' }} @endif>
                                    Publish</option>
                                <option value="pending" @if (isset($blog)) {{ $blog->status == 'pending' ? 'selected' : '' }} @endif>
                                    Pending</option>
                            </select>
                            @error('status')
                            <span class="text-danger"> {{ $message }} </span>
                    ...
                    <div class="form-group row mt-5">
                        ...
                        <div class="col-sm-10">
                            <select class="form-select  selectpicker " aria-label="Default select example" data-live-search="true" multiple name="tags[]">
                                <option value="default">Please select a status</option>
                                @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}" 
                                    @if(isset($blog))
