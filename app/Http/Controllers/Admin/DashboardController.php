...
class DashboardController extends Controller
{
    ...
    {
        $users =  $this->userObj->with('roles')->get();
        $userscount = $users->reject(function ($user, $key) {
            return $user->hasRole('admin');
        })->count();

        $categoryCount = $this->categoryObj->count();

        $blogCount = $this->postObj->count();



        $blogs = $this->postObj->selectRaw("COUNT(*) as count")
        ...
        $categories = $this->postObj->select('categories.name as category_name')
